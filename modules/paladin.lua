...
if config.enabled and addon.playerClass == "PALADIN" then
	...
	if config.righteousFury then
		addon:AddReminder("Missing Righteous Fury", function() return addon:InPVEInstance() and ((addon:HasTalent(2, 26) or UnitLevel("player") < 80) and not UnitAura("player", "Righteous Fury")) end, {type = "spell", spell = "Righteous Fury"})
	end
