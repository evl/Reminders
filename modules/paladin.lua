...
if config.enabled and select(2, UnitClass("player")) == "PALADIN" then
	...
	if config.righteousFury then
		evl_Reminders:AddReminder("Missing Righteous Fury", function() return not evl_Reminders:PlayerHasBuff("Righteous Fury") end, "Spell_Holy_SealOfFury", {type = "spell", unit = "player", spell = "Righteous Fury"})
	end
end