...
if config.enabled and addon.playerClass == "ROGUE" then
	...
	local throwingAttributes = {type = "item", ["target-slot"] = 18, item1 = getPoisonItemString(config.throwingPoisons[1]), item2 = getPoisonItemString(config.throwingPoisons[2]), threshold = config.thresholdTime}
