...
if config.enabled and addon.playerClass == "WARLOCK" then
	...
	addon:AddReminder("Missing armor", "UNIT_AURA", function() return not addon:HasAnyAura(config.armors) end, {type = "spell", spell1 = config.armors[1], spell2 = config.armors[2]})
	addon:AddReminder("Missing Soul Link", {"UNIT_AURA", "UNIT_PET"}, function() return addon:HasTalent(2, 9) and not UnitAura("player", "Soul Link") and hasDemon() end, {type = "spell", spell = "Soul Link"})
